    ...
    <div class="container mx-auto px-6 py-12 max-w-6xl">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-5 bg-white rounded-xl shadow-md p-6 border border-gray-100 h-fit">
                ...
                @if ($infoLibur)
                    <div
                        class="bg-red-50 border-2 border-dashed border-red-200 text-red-900 p-6 rounded-xl text-center my-6">
                        ...
                        <p class="text-xs text-gray-500 mt-3">Silakan ganti filter "Pilih Tanggal" di sebelah kanan untuk
                            mencari hari operasional lainnya.</p>
                    </div>
                @else
                    <form action="{{ route('booking.store') }}" method="POST" class="space-y-4">
                        ...
                        <!-- 🎯 TAMBAHKAN DROPDOWN PILIHAN TIPE PELANGGAN DI SINI -->
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Status Keanggotaan</label>
                            <select name="tipe_pelanggan" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm bg-white">
                                <option value="non-member" {{ old('tipe_pelanggan') == 'non-member' ? 'selected' : '' }}>
                                    Umum / Non-Member (Tarif Rp 30.000/jam)</option>
                                <option value="member" {{ old('tipe_pelanggan') == 'member' ? 'selected' : '' }}>Member
                                    Aktif GOR (Bebas Biaya Per Jam)</option>
                            </select>
                            <p class="mt-1 text-xs text-gray-400 italic">*Status member akan diverifikasi kembali oleh pemilik
                                saat menyetujui pesanan.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_main" value="{{ old('tanggal_main', $tanggalTerpilih) }}"
                                required
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm">
                        </div>
                        ...
                        <button type="submit"
                            class="w-full bg-indigo-900 hover:bg-indigo-950 text-white font-bold py-3 rounded-lg shadow-md transition tracking-wide mt-4 text-sm">
                            Ajukan Jadwal Sewa
                        </button>
                    </form>
                @endif
            ...
            <div class="lg:col-span-7 bg-white rounded-xl shadow-md p-6 border border-gray-100">
                <div
                    ...
                    <form action="{{ route('booking.create') }}" method="GET" class="flex items-center gap-2">
                        ...
                        <input type="date" name="tanggal_filter" value="{{ $tanggalTerpilih }}"
                ...
                <div class="space-y-3">
                    <div
                        class="bg-indigo-50 text-indigo-900 px-4 py-2.5 rounded-lg text-sm font-semibold flex justify-between items-center">
                    ...
                    @if ($bookings->isEmpty())
                        <div class="text-center py-12 border-2 border-dashed border-gray-200 rounded-xl">
                            ...
                            <p class="text-xs text-gray-400 mt-1">Belum ada yang booking pada tanggal ini. Silakan pilih jam
                                sesukamu.</p>
                        </div>
                    @else
                        <div class="grid grid-cols-1 gap-2">
                            @foreach ($bookings as $book)
                                <div
                                    class="flex items-center justify-between p-4 rounded-xl border border-red-100 bg-red-50/40">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="bg-red-500 text-white text-xs font-bold px-2.5 py-1.5 rounded-lg tracking-wider">
                                            TERISI
                                        </div>
                                        <div>
                                            <p class="text-sm font-bold text-gray-800">
                                                {{ date('H:i', strtotime($book->jam_mulai)) }} -
                                                {{ date('H:i', strtotime($book->jam_selesai)) }} WIB
                                            </p>
                                            <p class="text-xs text-gray-500 mt-0.5">Penyewa: {{ $book->nama_pelanggan }}
                                            </p>
                                        </div>
                                    </div>
                                    <span
                                        class="text-xs px-2 py-1 rounded bg-amber-100 text-amber-800 font-medium capitalized">
                                        {{ $book->status }}
                                    </span>
                                </div>
                            @endforeach
                ...
                <div class="mt-6 text-xs text-gray-400 bg-gray-50 p-3 rounded-lg leading-relaxed">
                    * Jadwal di atas diperbarui secara *real-time*. Jika status masih <span
                        class="text-amber-600 font-semibold">pending</span>, jam tersebut sudah di-tag namun menunggu
                    transfer pembayaran dikonfirmasi oleh pemilik GOR.
                </div>
            </div>
